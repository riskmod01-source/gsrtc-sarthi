...
// પ્રશ્ન મોડેલclass CompleteQuestion {
...
// સ્પેશિયલ ટેસ્ટ મોડેલclass SpecialExamModel {
  final String id;
  final String title;
...
// ૧. માસ્ટર રાઉટરclass MasterAppRouter extends StatefulWidget {
...
// ૨. લૉગિન પેજ (Firebase Email/Password + Google Sign-In)class CompleteLoginScreen extends StatefulWidget {
...
child: Text(isSignUp ? 'પહેલેથી એકાઉન્ટ છે? Log In' : 'નવા છો? એકાઉન્ટ બનાવો (Sign Up)', style: const TextStyle(fontSize: 12)),
),
const Row(
children: [
Expanded(child: Divider()),
Padding(padding: const EdgeInsets.symmetric(horizontal: 8), child: Text('અથવા', style: TextStyle(color: Colors.grey, fontSize: 12))),
Expanded(child: Divider()),
],
),
const SizedBox(height: 12),
...
icon: Image.network(
'wikimedia.org%22G%22_logo.svg/480px-Google%22G%22_logo.svg.png',
height: 20,
...
@override
State createState() => _CompleteProfileScreenState();
}
class _CompleteProfileScreenState extends State {
...
@override
State createState() => _RoleExamHubScreenState();
}
class _RoleExamHubScreenState extends State {
final Set _unlockedExamIds = {};
...
@override
State createState() => _ExamQuizScreenState();
}
class _ExamQuizScreenState extends State {
int currentIndex = 0;
int score = 0;
int? selectedAnswer;
Timer? _timer;
...
@override
State createState() => _CompleteAdminPanelState();
}
class _CompleteAdminPanelState extends State with SingleTickerProviderStateMixin {
...
Future _uploadGeneralBulkQuestions() async {
...
final docKey = '${bulkTargetRole}$_bulkSelectedSubject';
...
Future _uploadSpecialBulkTest() async {
...
Future _removeBookmark(int index) async {
final prefs = await SharedPreferences.getInstance();
final raw = prefs.getStringList('bookmarks') ?? [];
raw.removeAt(index);
...
// ══════════════════════════════════════════════
Future saveBookmark(CompleteQuestion question) async {
final prefs = await SharedPreferences.getInstance();
final raw = prefs.getStringList('bookmarks') ?? [];
